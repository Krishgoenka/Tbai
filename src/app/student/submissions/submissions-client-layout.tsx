...
interface SubmissionsClientLayoutProps {
  initialAssignments: Assignment[];
}

export function SubmissionsClientLayout({ initialAssignments }: SubmissionsClientLayoutProps) {
  const [assignments, setAssignments] = useState<Assignment[]>(initialAssignments);
  const [selectedAssignment, setSelectedAssignment] = useState<Assignment | null>(null);

  useEffect(() => {
    // When the initial assignments from the server change, update the state.
    // Also, select the first assignment by default if the list is not empty.
    setAssignments(initialAssignments);
    if (initialAssignments.length > 0) {
      setSelectedAssignment(initialAssignments[0]);
    } else {
